...
import { isProcTExp, makeBoolTExp, makeNumTExp, makeProcTExp, makeAnyTExp,makeNeverTExp, makeStrTExp, makeVoidTExp,
         parseTE, unparseTExp, makeUnionTExp,
         BoolTExp,  isBoolTExp,NumTExp, StrTExp, TExp, VoidTExp, isSubType,makeDiffTExp, isIsPredTExp, isNumTExp , isUnionTExp } from "./TExp";
...
export const L5typeofProgram = (concreteExp: string): Result<string> =>
    bind(p(concreteExp), (x) =>
        bind(parseL5Program(x), (exp: Program) => 
            bind(typeofProgram(exp, makeEmptyTEnv()), (texp: TExp) => {
                if (isUnionTExp(texp) && texp.components.length === 2 &&
                    (isNumTExp(texp.components[0]) && isBoolTExp(texp.components[1]) ||
                     isBoolTExp(texp.components[0]) && isNumTExp(texp.components[1]))) {
                    return unparseTExp(makeNumTExp());
                } else {
                    return unparseTExp(texp);
                }
            })));
...
export const typeofExp = (exp: Parsed, tenv: TEnv): Result<TExp> =>
    ...
    isVarRef(exp) ? 
        exp.var === "number" ? makeOk(makeNumTExp()) :
        exp.var === "boolean" ? makeOk(makeBoolTExp()) :
        exp.var === "string" ? makeOk(makeStrTExp()) :
        exp.var === "any" ? makeOk(makeAnyTExp()) :
        exp.var === "never" ? makeOk(makeNeverTExp()) :
...
export const typeofPrim = (p: PrimOp): Result<TExp> =>
    ...
    (p.op === 'number?') ? makeOk(makeProcTExp([makeAnyTExp()], makeBoolTExp())) :
    (p.op === 'boolean?') ? makeOk(makeProcTExp([makeAnyTExp()], makeBoolTExp())) :
    (p.op === 'string?') ? makeOk(makeProcTExp([makeAnyTExp()], makeBoolTExp())) :
...
export const typeofIf = (ifExp: IfExp, tenv: TEnv): Result<TExp> =>
    either(
        ...
        ({paramTE, returnTE, varRef}) => {
            if (!isAppExp(ifExp.test)) {
                return makeFailure("Test expression in if is not an application");
            }
            if (!isVarRef(ifExp.test.rator)) {
                return makeFailure("Predicate is not a variable reference");
            }
            const thenTEnv = makeExtendTEnv([varRef.var], [returnTE], tenv);
            const altTEnv = makeExtendTEnv([varRef.var], [makeDiffTExp(paramTE, returnTE)], tenv);
            return bind(typeofExp(ifExp.then, thenTEnv), (thenTE: TExp) =>
                bind(typeofExp(ifExp.alt, altTEnv), (altTE: TExp) => {
                    if (isNumTExp(thenTE) && isNumTExp(altTE)) {
                        return makeOk(makeNumTExp());
                    } else if (isSubType(thenTE, paramTE) && isSubType(altTE, paramTE)) {
                        return makeOk(paramTE);
                    } else {
                        return makeOk(makeUnionTExp([thenTE, altTE]));
                    }
                }));
        },
 ...
 const isTypePredApp = (exp: Exp, tenv: TEnv): Result<{paramTE: TExp, returnTE: TExp, varRef: VarRef}> => {
    if (isAppExp(exp)) {
        return bind(typeofExp(exp.rator, tenv), (ratorTE: TExp) => 
            isProcTExp(ratorTE) && ratorTE.paramTEs.length === 1 && 
            (isBoolTExp(ratorTE.returnTE) || isIsPredTExp(ratorTE.returnTE)) ?
                bind(typeofExp(exp.rands[0], tenv), (randTE: TExp) =>
                    isVarRef(exp.rands[0]) ?
                        makeOk({paramTE: randTE, returnTE: makeBoolTExp(), varRef: exp.rands[0]}) :
                        makeFailure("Argument to type predicate is not a variable reference")) :
                makeFailure("Not a type predicate application")
        );
    }
...
export const typeofProc = (proc: ProcExp, tenv: TEnv): Result<TExp> => {
    ...
    // Handle 'is?' predicate
    if (proc.returnTE.tag === "BoolTExp" && proc.body.length === 1 && isAppExp(proc.body[0])) {
        const bodyExp = proc.body[0];
        if (isPrimOp(bodyExp.rator) && 
            ["number?", "boolean?", "string?", "symbol?", "list?", "pair?"].includes(bodyExp.rator.op)) {
            return makeOk(makeProcTExp(argsTEs, makeBoolTExp()));
        }
    }
