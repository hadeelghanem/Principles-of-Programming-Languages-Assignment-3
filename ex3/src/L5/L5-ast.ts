...
const parseProcExp = (vars: Sexp, body: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        ...
        const returnTE: Result<TExp> = body[0] === ":" ? 
            (body[1] === "is?" ? 
                makeOk({ tag: "BoolTExp" }) : 
                parseTExp(body[1])) : 
            makeOk(makeFreshTVar());
        const bodyExps = body[0] === ":" ? body.slice(2) : body;
        const parsedBody = mapResult(parseL5CExp, bodyExps);
        return bind(args, (args: VarDecl[]) =>
                    bind(returnTE, (returnTE: TExp) =>
                        mapv(parsedBody, (body: CExp[]) =>
