...
export type TExp =  AtomicTExp | CompoundTExp | TVar |IsPredTExp;
...
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) ||
 isTupleTExp(x) || isUnionTExp(x)|| isInterTExp(x);
...
//=======Added========== 
export interface IsPredTExp {
    tag: "IsPredTExp";
}
//=======Added===========
export const makeIsPredTExp = (): TExp => {
    return { tag: "IsPredTExp" };
};
//=======Added===========
export const isIsPredTExp = (x: any): x is IsPredTExp => x.tag === "IsPredTExp";
...
const normalizeInter = (inter: InterTExp): TExp =>
    isEmpty(inter.components) ? makeAnyTExp() : 
...
// Implement makeDiffTExp
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp =>
    isAnyTExp(te1) ? te1 :
    isSubType(te1, te2) ? makeNeverTExp() :
    isUnionTExp(te1) ? makeUnionTExp(te1.components.map(te => makeDiffTExp(te, te2))) :
    isInterTExp(te1) ? makeInterTExp(te1.components.map(te => makeDiffTExp(te, te2))) :
    te1;
...
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ...
    isUnionTExp(te2) ? te2.components.some(t => isSubType(te1, t)) :
    (isInterTExp(te1) && isInterTExp(te2)) ? isSubset(te2.components, te1.components) :
    isInterTExp(te2) ? te2.components.every(t => isSubType(te1, t)) :
...
export const parseTExp = (texp: Sexp): Result<TExp> =>
    ...
    (texp === "is?") ? makeOk(makeIsPredTExp()) :  // Add this line
...
const parseCompoundTExp = (texps: Sexp[]): Result<TExp> =>
    ...
    (texps[0] === "inter") ? parseInterTExp(texps) :  // Add this line
    parseProcTExp(texps);
...
export const unparseTExp = (te: TExp): Result<string> => {
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isAnyTExp(x) ? makeOk('any') :  // Add this line
        isNeverTExp(x) ? makeOk('never') :  // Add this line
        isIsPredTExp(x) ? makeOk('is?') :  // Add this line
        ...
        isInterTExp(x) ? mapv(mapResult(unparseTExp, x.components), (componentTEs: string[]) => 
            parenthesizeInter(componentTEs)) :  // Add this line
...
const matchTVarsInTProcs = <T1, T2>(te1: TExp, te2: TExp,
    ...
    (isProcTExp(te1) && isProcTExp(te2)) ? 
        matchTVarsInTEs(procTExpComponents(te1), procTExpComponents(te2), succ, fail) :
    fail();
